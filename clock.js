let f=function(p,z=Intl.DateTimeFormat().resolvedOptions().timeZone){if(!(p&& p.clientHeight)) return; let qb=true, r2= 2*Math.PI; r0= Math.sin(r2/24); r0=r0/(1+r0)
 ...
 const x1=1100, c0=f(), c1=f(), c2=f(), t0=24*60*60*1000/286, t1=286, c=[c0.strokeStyle=c0.fillStyle='#111', c0.shadowColor='#FD941F'], r1=r0/5, q=[]
 f=function(p,z){q.push(p); p.addEventListener('dragstart',e=> e.preventDefault())
  p.addEventListener('click', _=>{if(qb) {qb=false; q.forEach(e=> e.dispatchEvent(new MouseEvent('click'))); qb=true} else f()})
  p.addEventListener('mouseover', _=>d1.src=d[db=0]); p.addEventListener('mouseout', _=>d1.src=d[db=1])
  ...
  let f=function(){let t=new Date(),t2=(Date.parse(t.toLocaleString('en',{timeZone:z}))+ t.getMilliseconds()- t.getTimezoneOffset()* 60000)/ t0
   ...
   ;[c0,c1].forEach(c=>{c.setTransform(1,0,0,1,0,0); c.lineWidth=2*r3; c.font=`bold ${r/4}px arial`; c.clearRect(0,0,x1,x1); c.translate(x,x)})
